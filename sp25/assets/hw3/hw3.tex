...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item $p \mid x_1 = 0, ..., x_r = 0 \propto \mathbb{P}(x_1 = 0, ..., x_r = 0 \mid p)f_p(p) \\
        \propto (1-p)^r p^{\alpha-1}(1-p)^{\beta-1}\mathbf{1}_{p \in [0, 1]} \\
        \propto p^{\alpha-1}(1-p)^{r+\beta-1}\mathbf{1}_{p \in [0, 1]} \\
        \implies p \mid x_1 = 0, ..., x_r = 0 \sim \text{Beta}(\alpha, r + \beta)$
        \item
        \begin{align*}
            \mathbb{P}(x_r = 0 \mid x_1=0, ..., x_{r-1} = 0) &= \int_0^1 \mathbb{P}(x_r = 0, p \in dp \mid x_1=0, ..., x_{r-1} = 0) \\ &*\mathbb{P}(p \in dp \mid x_1 = 0, ..., x_{r-1} = 0)\\
            &= \int_0^1 \mathbb{P}(x_r = 0 \mid p, x_1=0, ..., x_{r-1} = 0)f_{p \mid x_1=0, ..., x_{r-1}=0}(p) dp
            \end{align*}
            We use the fact that $p \mid x_1=0, ..., x_{r-1}=0 \sim \text{Beta}(\alpha, (r-1) + \beta)$, which was shown above. Continuing, we have
            \begin{align*}\\
            &= \int_0^1 (1-p) \frac{\Gamma(\alpha + \beta + r - 1)}{\Gamma(\alpha)\Gamma(\beta + r - 1)}p^{\alpha-1}(1-p)^{\beta + r - 2}dp \\
             \\
             &= \frac{\Gamma(\alpha + \beta + r - 1)}{\Gamma(\alpha)\Gamma(\beta + r - 1)} \int_0^1 p^{\alpha - 1}(1-p)^{\beta + r - 1}dp \\
             &= \frac{\Gamma(\alpha + \beta + r - 1)}{\Gamma(\alpha)\Gamma(\beta + r - 1)} \cdot \frac{\Gamma(\alpha)\Gamma(\beta + r)}{\Gamma(\alpha + \beta + r)} \\
             &= \frac{(\alpha + \beta + r-2)!}{(\beta + r-2)!} \cdot \frac{(\beta + r-1)!}{(\alpha + \beta + r-1)!} \\
             &= \boxed{\frac{\beta+r-1}{\alpha + \beta + r-1}} \\
        \end{align*}
        
        
        \item We will first solve this for any $\alpha, \beta$, then plug in $\alpha=1, \beta=2$.
        $$\mathbb{P}(x_1 = 0, ..., x_k = 0) = \mathbb{P}(x_1 = 0) \cdot \mathbb{P}(x_2 =0 \mid x_1=0) \cdot ... \cdot \mathbb{P}(x_k = 0 \mid x_1=0, ..., x_{k-1} = 0)$$
        Using the fact that $\mathbb{P}(x_r = 0 \mid x_1=0, ..., x_{r-1} = 0) = \frac{\beta+r-1}{\alpha + \beta + r-1}$, which was shown above, we get:
        $$\prod\limits_{r=1}^{12} \frac{\beta + r - 1}{\alpha + \beta + r - 1}$$
        $$ = \frac{2}{3} \cdot \frac{3}{4} \cdot ... \cdot \frac{13}{14} = \frac{2}{14} = \boxed{\frac{1}{7}}$$
    \end{enumerate}
    }
